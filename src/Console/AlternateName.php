...
use V360Tech\Geonames\Models\AlternateNamesWorking;

/**
 * Class AlternateName
 *
 * @package V360Tech\Geonames\Console
 */
class AlternateName extends AbstractCommand
{
  ...
  public function handle()
  {
    ...
    foreach ($urlsToAlternateNamesZipFiles as $countryCode => $urlsToAlternateNamesZipFile) {
      try {
        $absoluteLocalFilePathsOfAlternateNamesZipFiles[$countryCode] = $this->downloadFile(
          $this,
    ...
    foreach ($absoluteLocalFilePathsOfAlternateNamesZipFiles as $countryCode => $absoluteLocalFilePathOfAlternateNamesZipFile) {
      try {
        ...
        $this->error($e->getMessage());
        Log::error(
          $absoluteLocalFilePathOfAlternateNamesZipFile,
          $e->getMessage(),
          'local',
          $this->connectionName
        );
  ...
  protected function insertAlternateNamesWithLoadDataInfile($localFilePath): int
  {
    ...
    foreach ($localFileSplitPaths as $i => $localFileSplitPath) :
      ...
      try {
        ...
        throw new \Exception("Unable to execute the load data infile query. " . print_r(
          DB::connection($this->connectionName)
            ->getpdo()
            ->errorInfo(),
          TRUE
  ...
  protected function insertAlternateNamesWithEloquent($localFilePath): int
  {
    ...
    foreach ($localFileSplitPaths as $i => $localFileSplitPath) :
      ...
      foreach ($rows as $j => $row) {
        ...
        $alternateName = \V360Tech\Geonames\Models\AlternateNamesWorking::on($this->connectionName)
          ->firstOrCreate(
