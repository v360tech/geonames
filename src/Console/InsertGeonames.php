...
use V360Tech\Geonames\Models\Log;
...
class InsertGeonames extends AbstractCommand
{
  ...
  public function handle()
  {
    ...
    $absolutePathsToZipFiles = GeoSetting::getAbsoluteLocalStoragePathToFiles(
      $zipFileNames,
      $this->connectionName
    );
    ...
    try {
      ...
      Log::error(
        '',
        "We were unable to unzip at least one of the country zip files.",
        'local',
        $this->connectionName
      );
      throw $e;
  ...
  {
    $absolutePathToMasterTxtFile = GeoSetting::getAbsoluteLocalStoragePathToFile(
      $this->masterTxtFileName,
      $this->connectionName
    );
    ...
    if ($this->allCountriesInLocalTxtFiles($textFileNames)) {
      ...
      $absolutePathToAllCountriesTxtFile = GeoSetting::getAbsoluteLocalStoragePathToFile(
        $this->allCountriesTxtFileName,
        $this->connectionName
      );
      $renameResult                      = rename(
        $absolutePathToAllCountriesTxtFile,
...
SET created_at=NOW(),updated_at=null";
  ...
  /**
   * @param $localFilePath
   * @throws \V360Tech\LocalFile\Exceptions\UnableToOpenFile
   */
  protected function insertGeonamesWithEloquent($localFilePath)
  {
    ...
    try {
      ...
      foreach ($chunkedRows as $rowsToInsert) :
        \V360Tech\Geonames\Models\GeonameWorking::insert($rowsToInsert);
        $bar->advance();
      endforeach;
      $bar->finish();
    } catch (\Exception $exception) {
